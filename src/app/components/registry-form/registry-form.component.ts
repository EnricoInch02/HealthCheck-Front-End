...
export class RegistryFormComponent implements OnInit {
  ...
  model!: RegistryDTO;
  selectedCities!: Registry;
  mode: string = this.fullFormService.mode;

  constructor(private fullFormService: FullUserFormService) {}
  ngOnInit(): void {
    
    if (this.mode == 'insert') {
      this.model = new RegistryDTO(null);
      this.fullFormService.modelRegistry = this.model
    } else if(this.mode == "update") {
      this.model = new RegistryDTO(
           this.userUpdate!.registry.id,
        this.userUpdate!.registry.name,
        this.userUpdate!.registry.surname,
        (this.userUpdate!.registry.gender =
          this.userUpdate!.registry.gender == 'MASCHIO' ? '0' : '1'),
        this.userUpdate!.registry.birthDate,
        this.userUpdate!.registry.birthPlace,
        this.userUpdate!.registry.nationality,
        this.userUpdate!.registry.city,
        this.userUpdate!.registry.address,
        this.userUpdate!.registry.bg,
        this.userUpdate!.registry.cf,
        this.userUpdate!.id
      );
      this.fullFormService.modelRegistry = this.model
    }
  ...
  handleDateSelection(event: any) {
    this.model.birthDate = event.toISOString().split('T')[0];
  }
